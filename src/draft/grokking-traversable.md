...
Функция `createCheckout` возвращает тип `Checkout option`. Она возвращает значение `Some`, если все товары были зарезервированы и `None`, если некоторые товары зарезервировать не удалось. Серъезное решение должно бы было вернуть тип `Result` и детальное описание ошибки, но мы используем `Option` для простоты.
...
Проблема в том, что `reservedItems` имеет тип `list<option<ReservedBasketItem>>`, а нам надо, чтобы тип был `option<list<ReservedBasketItem>>`, где результат имеет значение `None`, когда, как уже было сказано, хотя бы один из элементов не удалось зарезервировать. Мы сможем рассчитать полную стоимость и создать `Checkout` только если все товары доступны. Представим, что мы написали такую функцию `reserveItems`, которая возвращает нам нужный тип.
...
Уже лучше! Теперь, если все элементы зарезервированы, функция возвращают значение `Some` и мы можем получить список `ReservedBasketItem`. Если какой-либо из элементов не может быть зарезервирован, мы получим значение `None` и функция `Option.map` прервет вычисление, стало быть `createCheckout` тоже вернет `None`.

Итак, мы свели задачу к реализации функции `reserveItems`. Уже понятно, что мы не можем просто использовать `List.map reserveBasketItem`, потому что типы обертки идут не в том порядке - сначала `list` и затем `option`. Нам надо придумать способ инвертировать их!
...
Как видите, тут нет ничего сложного. Мы проверяем оба аргумента, являются ли они значением `Some`, и если да, разворачиваем, объединяем при помощи оператора `::` и заворачиваем обратоно в `Some`. Во всех остальных случаях просто возвращаем `None`.
...
Строго говоря, тип, который является аппликативным функтором, также должен иметь метод `pure`. У `pure` очень простая задача - обернуть значение в тип контейнера. Например, в случае с `option`, `pure` это просто вызов `Some`.
...
Мы могли бы передать функцию `reserveBasketItem` в `sequence`, значет теперь у нас будет следующая сигнатура

```F#
let sequence (f: 'a -> 'b option) (list: 'a list): option<list<'b>>
```

Мы получаем список и пытаемся применить фугкцию `f` к каждому элементу этого списка. При этом, вместо того, чтобы отобразить все элементы списка и получить `list<option<'b>>`, мы сразу же хотим саггрегировать все значения в `option<list<'b>>`.
...
Мы можем поэксперементировать в интерактивном сеансе F# с использованием уже упомянутой библиотеки `FsharpPlus`
