...
Этот паттерн встрачается во множестве различных задач, и, извлекая функцию `map`, мы можем избавиться от довольно большого количества шаблонного кода. Например, в случае с `option` нам бы пришлось писать много условных выражений или pattern matching.
